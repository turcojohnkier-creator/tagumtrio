...
class DepartmentRequest(Base):
    ...
    requested_department: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="pending", index=True)
    requested_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    leadman_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    leadman_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
