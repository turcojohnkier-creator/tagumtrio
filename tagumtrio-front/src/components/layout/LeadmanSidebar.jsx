  ...
  return (
    <aside className={cn('fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-slate-800 bg-slate-950 transition-transform duration-300 md:static md:translate-x-0', mobileOpen ? 'translate-x-0' : '-translate-x-full')}>
      <div className="border-b border-slate-800 p-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-[0.3em] text-slate-500">{t('leadman.portal', 'Leadman Portal')}</p>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-4">
        <p className="px-3 pb-3 text-xs font-semibold uppercase tracking-widest text-slate-500">{t('ui.navigation', 'Navigation')}</p>
        {links.map((link) => (
          <NavLink
            key={link.to}
            to={link.to}
      ...
      <div className="border-t border-slate-800 p-4">
        <div className="mb-3 rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
          <p className="text-xs uppercase tracking-widest text-slate-500">{t('ui.signed_in', 'Signed in as')}</p>
