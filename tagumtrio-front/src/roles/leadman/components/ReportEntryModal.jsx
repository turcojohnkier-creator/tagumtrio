  ...
  return (
    <Portal>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <form onSubmit={handleSubmit} className="relative w-full max-w-full sm:max-w-md md:max-w-2xl lg:max-w-3xl xl:max-w-4xl rounded-lg border border-zinc-200 bg-white p-6 shadow-sm max-h-[90vh] overflow-auto">
          <button type="button" onClick={onClose} className="absolute right-5 top-5 text-zinc-500 hover:text-zinc-900">
            <X className="h-5 w-5" />
          </button>
          ...
          <div className="mt-5 space-y-4">
            ...
            {spec.fields
              ...
              .map((field) => (
                <div key={field.key}>
                  <label className="text-sm text-zinc-700">{field.label}</label>
                  <input
                    value={values[field.key] || ''}
                    onChange={(e) => handleChange(field.key, e.target.value)}
                    placeholder={field.placeholder || ''}
                    readOnly={field.type === 'date' || field.key === 'date'}
                    className={`mt-2 w-full rounded-xl border border-zinc-200 ${field.type === 'date' || field.key === 'date' ? 'bg-zinc-50 cursor-not-allowed' : 'bg-white'} px-3 py-2.5 text-zinc-900 focus:border-emerald-500 focus:outline-none`}
                  />
                </div>
              ))}
                        ...
                        {photoPreview[photoKey] ? (
                          <div className="relative w-full h-24 rounded">
                            ...
                            <button
                              ...
                              className="absolute top-1 right-1 bg-rose-500 rounded-full p-1 text-zinc-900 hover:bg-rose-600"
                            >
                              <X className="w-3 h-3" />
                            </button>
                          </div>
          ...
          <div className="mt-5 flex items-center justify-end gap-3">
            ...
            <Button type="submit" disabled={submissionEmployees.length === 0 || isSubmitting}>
