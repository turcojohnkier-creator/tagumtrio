...
export default function MainLayout() {
  ...
  const notificationItems = useMemo(() => {
    ...
    return (
      <div className="h-screen bg-white text-slate-800 flex flex-col overflow-hidden">
        <header className="h-16 bg-slate-50 border-b border-slate-200 flex items-center justify-between px-6 shrink-0 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <div className="bg-emerald-500 p-2 rounded-lg"><QrCode className="w-5 h-5 text-slate-900" /></div>
            <h1 className="text-lg font-bold text-slate-900">TriOPS</h1>
          </div>
          <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="hidden sm:flex items-center gap-2">
                {headerCounts.map((item) => (
                  <div key={item.label} className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs text-slate-700">
                    <span className="text-slate-400">{item.label}</span> <span className="text-slate-900 font-semibold">{item.value}</span>
                  </div>
                ))}
              </div>
            )}
            <div className="relative">
              <button onClick={() => setNotificationsOpen((open) => !open)} className="relative p-2 text-slate-500 hover:text-slate-900 transition-colors" aria-expanded={notificationsOpen} aria-label="Toggle notifications">
              ...
              {notificationsOpen && (
                <div className="absolute right-0 top-12 z-30 w-[320px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-2xl shadow-black/30">
                  <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
                    <div>
                      <p className="text-sm font-semibold text-slate-900">Notifications</p>
                      <p className="text-xs text-slate-400">Pending items and latest updates</p>
                    </div>
                    <button onClick={() => setNotificationsOpen(false)} className="text-slate-500 hover:text-slate-900">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                    ...
                    {notificationItems.length === 0 ? (
                      ...
                      <div className="divide-y divide-slate-200">
                        {notificationItems.map((item, index) => (
                          <div key={`${item.title}-${index}`} className="px-4 py-3 hover:bg-white transition-colors">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={handleLogout} className="text-slate-500 hover:text-rose-700 transition-colors p-2" title="Sign Out"><LogOut className="w-5 h-5" /></button>
            </div>
          </div>
        </header>
  ...
  return (
    <div className="h-screen bg-white text-slate-800 flex overflow-hidden">
      ...
      <aside className={cn('fixed md:static inset-y-0 left-0 z-30 w-64 md:h-screen bg-slate-50 border-r border-slate-200 flex flex-col transition-transform duration-300', mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0')}>
        <div className="p-6 flex items-center justify-between md:justify-start gap-3 border-b border-slate-200">
          <div className="flex items-center gap-3">
            <div className="bg-emerald-500 p-2 rounded-lg shadow-lg shadow-emerald-500/20"><QrCode className="w-6 h-6 text-slate-900" /></div>
            <div>
              <h1 className="text-xl font-bold text-slate-900">TriOPS</h1>
              <p className="text-[10px] text-slate-500 font-medium uppercase tracking-wider mt-0.5">{user.role}</p>
            </div>
          </div>
          <button className="md:hidden p-2 text-slate-500" onClick={() => setMobileMenuOpen(false)}><X className="w-5 h-5" /></button>
        </div>

        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4 mt-2 px-3">Navigation Menu</div>
        ...
        <div className="p-4 border-t border-slate-200">
          <div className="flex items-center gap-3 px-3 py-3 rounded-lg bg-slate-100/50 mb-3 border border-slate-300/50">
            <div className="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-700 font-bold border border-emerald-500/20">{user.name.split(' ').map(n=>n[0]).join('').substring(0,2)}</div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-900 truncate">{user.name}</p>
              <p className="text-xs text-slate-500 truncate capitalize">{user.role.replace('_',' ')}</p>
            </div>
          </div>
          <button onClick={handleLogout} className="flex items-center gap-3 w-full px-3 py-2 text-sm font-medium text-slate-500 hover:text-rose-700 hover:bg-rose-500/10 rounded-lg transition-colors"><LogOut className="w-5 h-5" />Sign out</button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-slate-50 px-4 md:px-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setMobileMenuOpen((open) => !open)} className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white p-2 text-slate-700 md:hidden" aria-label="Toggle navigation">
              {mobileMenuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
            </button>
            <div>
              <p className="text-sm font-semibold text-slate-900">Workspace</p>
              <p className="text-xs text-slate-400">Dashboard & navigation</p>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <div className="relative">
              <button onClick={() => setNotificationsOpen((open) => !open)} className="relative p-2 text-slate-500 hover:text-slate-900 transition-colors bg-white rounded-lg border border-slate-200" aria-expanded={notificationsOpen} aria-label="Toggle notifications">
                <Bell className="w-5 h-5" />
                {notificationItems.length > 0 && (
                  ...
                  {notificationsOpen && (
                    <div className="absolute right-0 top-12 z-30 w-[340px] overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-2xl shadow-black/30">
                  <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
                    <div>
                      <p className="text-sm font-semibold text-slate-900">Notifications</p>
