          ...
          <div className="flex items-start justify-between gap-4 border-b border-zinc-200 px-5 py-4">
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                <h3 className="font-heading text-lg font-bold text-zinc-900">{report.department || 'Unknown Department'}</h3>
                <Badge variant={getStatusVariant(report.status)}>{getStatusLabel(report.status)}</Badge>
              </div>
              <p className="mt-1 text-sm text-zinc-500">Submitted by {report.submittedByName || report.submittedBy || 'Unknown'} • {formatDateTime(report.createdAt || report.created_at || report.reportDate)}</p>
            </div>
        ...
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/75 p-4">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-xl border border-zinc-200 bg-white shadow-sm p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setPreviewPhotos(null)} className="text-zinc-500 hover:text-zinc-900">Close</button>
            </div>
